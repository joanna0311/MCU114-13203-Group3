...
class AddHabitFragment : Fragment() {
    ...
    private fun setupViewListeners() {
        binding.tilHabitTitle.editText?.doOnTextChanged { text, _, _, _ ->
            if (!isUpdatingFromObserver && habitViewModel.editingHabit.value?.name != text.toString()) {
                updateHabit { it.copy(name = text.toString()) }
            }
        }

        binding.tilHabitNotes.editText?.doOnTextChanged { text, _, _, _ ->
            if (!isUpdatingFromObserver && habitViewModel.editingHabit.value?.notes != text.toString()) {
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        binding.actvRepetitionType.setAdapter(null)
        _binding = null
    }
}
